...
class SwapRateApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Rate Edge Data Manager - Professional Edition 7.1")
        self.root.geometry("1400x900")
        
        # Set window icon
        try:
            from PIL import Image, ImageTk
            icon_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'resources', 'assets', 'logo.png')
        ...
        # Modern color scheme - Professional Blue/Grey
        self.colors = {
            ...
            'danger': '#e74c3c',         # Red
            'bg_main': '#ecf0f1',        # Light grey background
            'bg_card': '#ffffff',        # White for cards
            'text_dark': '#2c3e50',      # Dark text
            'text_light': '#7f8c8d',     # Light grey text
    ...
    def setup_ui(self):
        ...
        header_frame = tk.Frame(main_container, bg='#003554', height=90)
        ...
        center_container = tk.Frame(header_frame, bg='#003554')
        center_container.pack(expand=True, fill=tk.BOTH)
        
        # Inner frame to hold logo and text (centered)
        inner_container = tk.Frame(center_container, bg='#003554')
        inner_container.pack(anchor=tk.CENTER)
        
        # Try to load and display logo
        try:
            from PIL import Image, ImageTk
            logo_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'resources', 'assets', 'logo.png')
            if os.path.exists(logo_path):
                logo_img = Image.open(logo_path)
                # Resize logo to fit header (maintain aspect ratio)
                logo_img = logo_img.resize((90, 90), Image.Resampling.LANCZOS)
                logo_photo = ImageTk.PhotoImage(logo_img)
                logo_label = tk.Label(inner_container, image=logo_photo, bg='#003554')
                logo_label.image = logo_photo  # Keep a reference
                logo_label.pack(side=tk.LEFT, padx=(0, 15), pady=0)
        except Exception as e:
            print(f"Could not load logo: {e}")
        
        # Title container
        title_container = tk.Frame(inner_container, bg='#003554')
        title_container.pack(side=tk.LEFT, fill=tk.Y)
        
        # Main title
        title_label = tk.Label(title_container, 
                              text="Rate Edge Data Manager", 
        ...
        button_frame = tk.Frame(header_frame, bg='#003554')
        ...
        bm_curr_combo.bind('<<ComboboxSelected>>', lambda e: self.update_benchmark_rate_type_list())
        ...
        ttk.Button(bm_filter_frame, text="📊 Table View", command=self.toggle_benchmark_view, style='Accent.TButton').grid(row=0, column=10, padx=(20, 5))
        ...
        ttk.Button(ois_filter_frame, text="📊 Table View", command=self.toggle_ois_view, style='Accent.TButton').grid(row=0, column=10, padx=(20, 5))
        
        # OIS table
        ois_table_card = ttk.Frame(ois_tab, style='Card.TFrame', relief='solid', borderwidth=1)
        ois_table_card.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        ois_table_frame = ttk.Frame(ois_table_card, style='Card.TFrame')
        ois_table_frame.pack(fill=tk.BOTH, expand=True, padx=3, pady=3)
        ois_table_frame.columnconfigure(0, weight=1)
        ois_table_frame.rowconfigure(0, weight=1)
        
        ois_columns = ('Date', 'Currency', 'Rate Type', 'Rate')
        self.ois_tree = ttk.Treeview(ois_table_frame, columns=ois_columns, show='headings', height=22, style='Treeview')
        
        self.ois_tree.heading('Date', text='📅 Date')
        self.ois_tree.heading('Currency', text='💱 Currency')
        self.ois_tree.heading('Rate Type', text='📊 Rate Type')
        self.ois_tree.heading('Rate', text='📈 Rate (%)')
        ...
        ois_vsb = ttk.Scrollbar(ois_table_frame, orient="vertical", command=self.ois_tree.yview)
        ois_hsb = ttk.Scrollbar(ois_table_frame, orient="horizontal", command=self.ois_tree.xview)
        self.ois_tree.configure(yscrollcommand=ois_vsb.set, xscrollcommand=ois_hsb.set)
        
        self.ois_tree.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        ois_vsb.grid(row=0, column=1, sticky=(tk.N, tk.S))
        ois_hsb.grid(row=1, column=0, sticky=(tk.W, tk.E))
        
        self.ois_tree.tag_configure('evenrow', background=self.colors['bg_card'], foreground=self.colors['text_dark'])
        self.ois_tree.tag_configure('oddrow', background=self.colors['table_alt'], foreground=self.colors['text_dark'])
        ...
        ttk.Button(io_frame, text="📥 Import Excel", 
                  command=self.import_excel, style='Accent.TButton').pack(side=tk.LEFT, padx=3)
    ...
    def refresh_pivot_view(self, filters=None):
        ...
        try:
            ...
            # ALSO FETCH 3M BENCHMARK RATES (for currencies that need them)
            # For AUD: BBSW_3M, NZD: BKBM_3M, CAD: Term_CORRA_3M
            benchmark_sql = """
                SELECT date, currency, rate FROM benchmark_rates 
                WHERE (currency = 'AUD' AND rate_type = 'BBSW_3M')
                   OR (currency = 'NZD' AND rate_type = 'BKBM_3M')
                   OR (currency = 'CAD' AND rate_type = 'Term_CORRA_3M')
            """
            ...
            # Second, add 3M benchmark rates
            for date, currency, rate in benchmark_rows:
                date_norm = normalize_date(date)
                key = (date_norm, currency)
                if key not in data_dict:
                    data_dict[key] = {}
                # Add as '3M' tenor - benchmark rates are already in percentage format
                # (e.g., 3.6354 means 3.6354%, not 0.036354)
                # So we don't multiply by 100 like we do for swap rates
                data_dict[key]['3M'] = rate
            ...
            for i, (date, currency) in enumerate(sorted_keys):
                ...
                for tenor in tenor_columns:
                    if tenor in data_dict[(date, currency)]:
                        values.append(f"{data_dict[(date, currency)][tenor]:.4f}")
                    else:
                        values.append("")
    ...
    def show_about(self):
        """Show about dialog"""
        messagebox.showinfo(
            "About Rate Edge Data Manager",
            "Rate Edge Data Manager\n\n"
...
"""
    ...
    def refresh_benchmark_data(self, currency=None, rate_type=None, start_date=None, end_date=None):
        ...
        for i, rate in enumerate(rates):
            ...
            self.benchmark_tree.insert('', tk.END, values=(
                date_str, rate[1], rate[2], f"{rate[3]:.4f}"
            ), tags=tags)
    ...
    def refresh_benchmark_pivot_view(self):
        ...
        for row in rows:
            ...
            pivot_data[(date, currency)]['O/N' if tenor in ['1D','ON'] else tenor] = f"{rate:.4f}"
        for i, ((date, currency), values) in enumerate(sorted(pivot_data.items(), reverse=True)):
            row_values = [date, currency] + [values.get(c, '') for c in ['O/N','1M','2M','3M','4M','5M','6M']]
    ...
    def refresh_ois_pivot_view(self):
        ...
        for row in rows:
            ...
            if tenor in all_tenors:
                pivot_data[(date, currency)][tenor] = f"{rate:.4f}"
    ...
    def refresh_ois_standard_view(self):
        ...
        for i, row in enumerate(rows):
            ...
            self.ois_tree.insert('', 'end', values=(date, currency, f"{floating_rate} {tenor}", f"{rate:.4f}"), 
                               tags=('evenrow' if i%2==0 else 'oddrow',))
        self.ois_tree.tag_configure('evenrow', background=self.colors['bg_card'])
        self.ois_tree.tag_configure('oddrow', background=self.colors['table_alt'])
